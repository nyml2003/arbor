...
- 状态：已批准
- 批准日期：2026-07-11
- 实现状态：`S0`、三个 `F1` lane、`B1` 与 `PT1` 已完成；Terminal Unicode 文本合同已通过，GPU adapter 仍在 `F2`；上层组件候选尚未进入实现 wave
...
- GPU adapter。当前继续实现 logical planner、wgpu runtime 与 Tetris GPU 入口；固定环境 readback 需要 `GPU-REF-v0.1` 通过。
