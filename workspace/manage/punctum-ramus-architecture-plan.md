...
- 状态：已批准
- 批准日期：2026-07-11
- 实现状态：`S0`、三个 `F1` lane、`B1`、`PT1`、`F2`、`R2`、`F3a`、`B2a`、`B2b`、`T2` 与 `F3b` 已完成；下一步执行 `F3d/F3e/B3a`，以真实 Game GUI 证据收窄公共边界
- 评审结果：Planner、Architect、Critic 共识通过，最终 Critic 结论为 `APPROVE`
- 产品事实来源：[项目群总控](./punctum-ramus-program.md)

## 文档职责

本文只定义稳定架构：共享边界、workspace 拓扑、依赖方向、合同和验证原则。执行 DAG、Agent Prompt、写入范围和当前门禁见[第一期执行计划](./punctum-ramus-execution-plan.md)。
...
概念分层不等于一个层级对应一个 crate。当前物理结构为单一 `punctum-ui` pure crate。F3b 先实现 Primitive；F3c 在同一 crate 增加 provisional interaction。产品 Shared Pattern 与 Feature Component 留在消费方。只有独立依赖、编译成本、API 生命周期或多个消费者证明需要物理隔离时，才评估拆为 `punctum-ui-core` 和 `punctum-widgets`。
...
完整 UI foundation 不得插入已经完成的 adapter lane。`F3a` 只修复现有 Terminal 输出 API 的职责错误，不创建 `punctum-ui`，因此可以与 `B2a` 并行。`B2b` 完成 Terminal/GPU 基础验证后启动 `F3b`。用户在执行中取消 `PEP 0002`；`F3b` 合同记录在 crate rustdoc、类型边界和合同测试中。
...
Terminal 只读文本投影已经完成 grapheme 分割、Unicode width、continuation cell、覆盖修复、裁剪、resize 清理和 cursor 停放，并由 `punctum-terminal` 的 19 项纯合同测试验证。该结果不等于共享 `Text` Primitive 已完成。

F3a 前的 `write_text` 接收输入侧 `TextEvent`。F3a 已把输出边界改为 `&str`，并删除 `punctum-terminal` 对 `punctum-input` 的依赖。
...
`F3b` 已实现 `Text` Primitive。`Text` 只接收内容，不接收 `TextEvent`。`Ui::measure` 通过 backend `TextLayouter` 生成布局结果，`Measured::layout` 只放置节点，`Frame::paint` 复用同一结果并通过 `PaintTarget` 输出裁剪后的绘制操作。resize 使用新 constraints 重新执行同一无状态 pipeline。主题、字体和边框外观留在 backend。
...
### 开放问题：离散 Grid UI 与 Native GPU GUI

`F3b` 只证明了共享结构可以在整数离散空间中完成 measure、layout、clip 和 paint。它没有证明 Terminal GUI 与 Native GPU GUI 应共享同一种最终几何，也没有证明逻辑槽、Terminal Cell 和 GPU instance 应保持一一对应。

现有实现已经显示三种不同单位：

- `Surface<T>` 的槽位只表达离散坐标，不承诺终端列数或像素尺寸。
- `TerminalCell` 表达一个终端列。宽 grapheme 占两个列槽，并使用 continuation 维护占用关系。
- 当前 `GpuCell` 和 shader 表达固定大小的 grid/tile quad。Tetris 的一个逻辑棋盘格在 Terminal 投影为 `2 x 1` 列，在 GPU 投影为一个方形 quad。

因此，`GpuCell` 不能扩张为文字、图片、tile 和一般 GUI 的万能 Cell。文字在 Terminal 中需要列宽和 continuation；GPU 文字需要 backend text layout 和 glyph run。地图 tile 可以继续使用固定 grid；普通 GUI 图片需要独立的目标矩形和适配策略，不能默认与文字使用同一个 Cell 大小。

当前先接受以下不变量：

1. `punctum-grid` 和 `Surface<T>` 继续服务离散 grid、地图、棋盘和 tile layer。
2. `TerminalCell`、逻辑 grid slot 和 GPU draw instance 是不同概念，不建立公共一一映射。
3. `Text` 与 `SurfaceView` 保持不同 paint primitive。文字测量和最终投影由 backend 负责。
4. Tetris 可以继续使用当前整数布局验证共享信息层级，但该结果不能冻结 Native GPU GUI 合同。

进入 Game 的 Native GPU UI 实现前，必须通过独立设计任务回答：

1. `punctum-ui` 是否明确定位为离散 Grid UI，并为 Native GPU GUI 建立独立几何与 paint layer。
2. 如果继续共享布局算法，layout geometry 如何区分 Terminal 行列与 GPU logical pixel，避免把两种单位放进同一个 `GridSize` 语义。
3. GUI 共享边界是否只包含 widget tree、布局意图、focus、event 和 action，而允许两个 backend 产生不同 measure 和最终 bounds。
4. GPU 是否新增 `TextRun`、`ImageQuad`、`FillRect` 等 primitive，并把当前 `GpuCell` 限定在 grid/tile pipeline。

该问题现在阻塞 `T3` 和 `F3c`。原因不是 Tetris 无法继续，而是继续给当前离散布局增加页面和交互会扩大未经验证的公共 API。执行计划先通过 `F3d` 建立纯 Game screen model，再通过 `F3e` 在 Game 内验证 Native GPU GUI。两个任务不得修改 Punctum 公共 crate。`B3a` 只根据实现证据决定共享边界，并重写后续任务。

`F3e` 的 GPU GUI 使用整数虚拟像素，不使用 `GridSize` 表达 GUI 几何。虚拟像素到 framebuffer 的缩放、letterbox 和 DPI 转换属于 host。文字、图片、矩形和 grid view 使用不同 paint primitive。第一期仍不引入鼠标、触屏或任意连续布局。

`B3a` 必须明确以下结果：

1. `punctum-ui` 是保留为离散 Grid UI、拆分为语义层与 backend layout，还是停止向公共层提升。
2. 哪些类型可以稳定共享，哪些类型必须留在 Terminal 或 GPU adapter。
3. `WidgetId`、focus、event dispatch 和 action 是否具有与几何无关的共同合同。
4. `T3` 是否共享组件树，还是只共享 screen model、信息层级和 action。
5. Game GPU GUI 的文字、图片、矩形和 grid 嵌入由哪个 crate 拥有。

门禁禁止把“不重复代码”本身当作抽取依据。只有两个实现具有相同输入、状态转换和可原样复用的 black-box oracle 时，才能提升。布局代码因单位和宿主约束不同而重复，不自动构成偶然复杂度。
...
5. 后续稳定化评审明确 component identity、state ownership、layout overflow、text clipping 和 error model。
6. “离散 Grid UI 与 Native GPU GUI”开放问题已经解决，并由 Game 的真实文字、图片和 grid 组合界面验证。
...
- 第一阶段不做 hold、next preview、level、score、lock delay、音效或动画。

2026-07-12 用户批准增加 Ghost Piece，帮助玩家判断 hard drop 落点。预测从当前活动方块纯计算得到，不写入 `TetrisState`，不参与碰撞、锁定或消行。活动方块已位于预测落点或游戏已经结束时不显示 Ghost Piece。hard drop 的实际落点必须与预测位置一致。

`paint` 固定产生 12 x 22 的 `Surface<TetrisCell>`：10 x 20 棋盘外加一格边框。投影需要区分 empty、border、locked tetromino、Ghost Piece 和 active tetromino。绘制顺序固定为 locked、ghost、active，活动方块始终覆盖预测影子。
...
- 当前 DAG、Prompt 和门禁：[第一期执行计划](./punctum-ramus-execution-plan.md)
- 当前已完成 `T2` 与 `F3b`。下一步为 `F3d Game Screen Model`、`F3e Native GPU GUI Probe` 和 `B3a UI Boundary Decision`。`T3/F3c` 暂停等待 `B3a`。
- `GPU-REF-v0.1` 固定环境 readback 仍为 Blocked，但不阻塞本地实现。

## 计划变更规则

- 产品事实变化时，先更新项目群总控，再评估本文。
- shared core 扩张必须经过 extraction gate 和新 ADR。
- 执行任务必须保持原子，写入范围必须显式且不与并行节点重叠。
- root manifest、lockfile、path dependency 或 composition 的跨 lane 接收必须建立独立汇合节点。
