...
- 状态：已批准
- 批准日期：2026-07-11
- 实现状态：`S0`、三个 `F1` lane、`B1`、`PT1` 与 `F2` 已完成；Terminal Unicode 文本、GPU planner/runtime、winit 输入适配和本机 headless smoke 已通过；下一步先规划 `R2` adapter crate 边界，再进入 `B2`
...
上层 UI 候选不得插入已经完成的 `F2` adapter lane。`B2` 完成 Terminal/GPU 基础验证后，才允许建立新的 ADR 和实现 task。
...
`F2` 已完成，现有行为和测试作为 `R2` 的迁移基线。`R2` 位于 `F2` 与 `B2` 之间，只调整物理边界和验证政策，不增加功能，也不改变 Terminal/GPU 可见行为。

`R2` 必须形成以下结构：

1. 当前 `punctum-terminal` 保留 `TerminalCell`、Unicode 文本处理、resize、patch planning 和其他纯逻辑，不再依赖 Crossterm。
2. Crossterm raw event 转换、presenter、session 和真实终端 IO 移入独立平台 crate。最终 crate 名称在 `R2` 实施任务批准时确定。
3. 当前 `punctum-gpu` 保留 atlas、viewport、cell、submission planning、instance encoding 和 uniform encoding，不再依赖 winit 或 wgpu。
4. winit event 转换、wgpu runtime、shader、pipeline、surface 和 device 操作移入独立平台 crate。最终 crate 名称在 `R2` 实施任务批准时确定。
...
`R2` 的具体写入所有权、Agent 编排、crate 名称和实施时间由后续独立任务确定。本文不授权立即修改代码。`R2` 未通过前不得启动 `B2`。

### `B2`：串行 barrier

`F2` handoff 已通过，但 `B2` 仍等待 `R2`。`R2` 通过后，Program Integration Agent 才能接受 root manifest、lockfile 和 Tetris GPU composition。它执行以下动作：

1. 接受 Tetris 对纯 GPU contract、独立平台 adapter、winit 和 host-local async executor 的 dev-dependency，刷新相关 lockfile。实际 crate 名称使用 `R2` 批准结果。
...
| `R2` | 后续单独确定 | 后续单独确定 | 本文只固定边界和门禁，不提前派发 |
...
`F2` adapter lane 已完成。下一轮先为 `R2` 单独确定 crate 名称、任务范围、写入所有权和验证方式；本文不提供执行 Prompt，也不提前指定 Agent。`R2` 通过前不得启动 `B2`。
...
| `R2 Adapter Crate Boundary` | 已规划，未开始 | `F2` 已通过；等待后续单独确定实施任务和 crate 名称 |
| `B2 Dual Backend Tetris` | 未开始 | 等待 `R2` 通过后再接通 GPU 入口并运行双后端 smoke |
