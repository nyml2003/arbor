...
- 当前阶段：`F2` 已开始；Terminal Unicode 文本能力已完成，GPU adapter 仍在实现
- 本轮记录：Terminal 已支持 grapheme、Unicode width、continuation、宽字符覆盖修复、裁剪、resize 清理和 cursor 停放；TUI Chater 暂停，Tetris 改为当前基座验证项目
- 更新日期：2026-07-12
...
| 项目 | 产品职责 | 当前事实 |
| --- | --- | --- |
| Punctum | 可复用 UI 核心及平台适配 | grid/input、Terminal Tetris 与 Terminal Unicode 文本已完成；GPU adapter 仍在实现 |
...
| `T1 Tetris Foundation Proof` | Tetris 通过 Terminal/GPU 验证 adapter、文本与后续 UI 基座 | Terminal 文本已通过；等待 GPU 与上层 UI |
...
- 本轮不建设 CI。`PT1` 已通过，Terminal Unicode 文本合同的 28 项测试已通过；`F2` 的剩余主线是 GPU adapter 与 Tetris GPU 入口。
