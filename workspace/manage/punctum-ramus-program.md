...
- 当前阶段：`F2` 收尾；Terminal Unicode 文本和 GPU 本地 planner/runtime/smoke 已完成，仍需完成 GPU 输入适配和 Tetris GPU 入口
- 本轮记录：下一阶段固定按 `F2 -> B2 -> F3 -> B3` 推进；先完成双后端 Tetris，再建立 provisional UI foundation
- 更新日期：2026-07-12
...
| 项目 | 产品职责 | 当前事实 |
| --- | --- | --- |
| Punctum | 可复用 UI 核心及平台适配 | grid/input、Terminal 与 GPU 本地基础已完成；GPU 输入适配、Tetris GPU 入口和上层 UI 尚未完成 |
...
1. `F2` 完成 GPU 键盘事件规范化，并冻结本地 GPU adapter 合同。
2. `B2` 由 Program Integration Agent 接通 Tetris GPU 入口，完成 Terminal/GPU 双后端本地验收。
3. `F3` 先批准 provisional UI `PEP 0002`，再实现无状态布局、只读文本、焦点和基础 widget。
4. `B3` 接受 Punctum 与 Tetris 的 UI 依赖变化，验证 UI crate 不依赖 backend 或产品类型。

`GPU-REF-v0.1` 继续阻塞固定环境 readback 和正式 release oracle，但不阻塞 `F2` 本地 adapter、`B2` 双后端 Tetris 或 `F3` UI foundation。

`F2` 和 `F3` 各保留一个主实现 writer。其他并发席位用于独立验证、架构评审和 Ramus branch coverage 验证器，不并行修改同一公共合同。真正的两个业务实现 lane 从 `F4` 开始：Game UI 与 Battle/Ramus/Agent harness 可以并行，composition 和 E2E 继续串行。
...
- 本轮不建设 CI。`PT1` 已通过，Terminal Unicode 文本合同的 28 项测试已通过，GPU pure planner、wgpu runtime 和本机 headless smoke 已通过。`F2` 还需完成 winit 键盘事件规范化；随后在 `B2` 接通 Tetris GPU 入口。
