...
- 状态：产品需求和第一期架构已批准；`S0`、三个 `F1` lane、`B1`、`PT1`、`F2`、`R2` 与 `B2a` 已完成
- 当前阶段：`B2a` Tetris GPU 已完成；等待 `F3a` 完成后进入单一汇合任务 `B2b`
- 本轮记录：Tetris GPU 已接通 winit、wgpu、整数 viewport、完整首帧和后续 diff patch；后续按 `F3a -> B2b -> F3b -> F3c -> B3` 推进
- 更新日期：2026-07-12
...
| 项目 | 产品职责 | 当前事实 |
| --- | --- | --- |
| Punctum | 可复用 UI 核心及平台适配 | grid/input、Terminal、GPU 与平台 adapter crate 拆分已完成；Tetris GPU 入口已完成，Terminal 文本边界和上层 UI 仍待后续任务 |
...
| `T1 Tetris Foundation Proof` | Tetris 通过 Terminal/GPU 验证 adapter、文本与后续 UI 基座 | Terminal 与 GPU 入口已通过各自本地验证；等待 `B2b` 双后端验收与上层 UI |
...
- manifest、lockfile、composition 和跨域 E2E 由 DAG 节点按文件独占。所有权只在依赖边上传给后续汇合任务，不由常驻 Agent 角色跨任务持有。
- 实现按 `S0 -> F1 -> B1 -> PT1 -> F2 -> R2 -> (F3a || B2a) -> B2b -> F3b -> F3c -> B3 -> F4 -> F5` 推进。
...
3. `B2a` 已完成 Tetris GPU 入口、纯 projection、整数 viewport、首帧完整提交和后续 patch。`F3a` 继续独立修复 Terminal 文本边界。
4. `B2b` 在 `F3a` 完成后接收两个上游任务的 manifest、lockfile、验证注册和状态文档，完成双后端本地验收。
5. `F3b` 先批准 `PEP 0002`，再实现无状态布局、只读 `Text` 和 Terminal/GPU 文本投影。
6. `F3c` 在无状态布局稳定后实现焦点、事件分发和 `Button`。没有 Game 真实需求时不实现 `ChoiceList`、`List` 或 `ScrollView`。
7. `B3` 接受 Punctum 与 Tetris 的 UI 依赖变化，验证 UI crate 不依赖 backend 或产品类型。

`GPU-REF-v0.1` 继续阻塞固定环境 readback 和正式 release oracle，但不阻塞 `B2a/B2b` 双后端 Tetris 或 `F3b/F3c` UI foundation。
...
- 本轮不建设 CI。`R2` 已把 19 项 Terminal 纯合同与 9 项 Crossterm 合同分开，把 GPU planner 和字节编码留在纯 crate，并把 8 项 winit 输入合同与 headless smoke 移入 `punctum-wgpu`。下一步并行执行 `F3a` 与 `B2a`，再由 `B2b` 接收。
