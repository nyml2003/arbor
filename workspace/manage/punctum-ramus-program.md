...
- 状态：产品需求和第一期架构已批准；`S0`、三个 `F1` lane、`B1`、`PT1` 与 `F2` 已完成
- 当前阶段：等待 `R2`；Terminal Unicode 文本、GPU planner/runtime、winit 输入适配和本机 smoke 已完成，adapter 副作用边界尚待按 crate 拆分
- 本轮记录：下一阶段固定按 `R2 -> B2 -> F3 -> B3` 推进；先完成 adapter crate 边界，再接通双后端 Tetris 和 provisional UI foundation
- 更新日期：2026-07-12
...
纯逻辑与副作用的边界必须由 crate 表达，不能长期依赖文件名或 `runtime.rs` 排除规则。`R2` 将在不改变产品行为的前提下整理这一边界。具体 crate 名称、实施任务和 Agent 编排在后续独立规划中确定。

## 项目群

| 项目 | 产品职责 | 当前事实 |
| --- | --- | --- |
| Punctum | 可复用 UI 核心及平台适配 | grid/input、Terminal、GPU 本地基础与 winit 输入适配已完成；Tetris GPU 入口和上层 UI 尚未完成 |
...
2. `R2` 规划并验证 adapter 的纯逻辑 crate 与平台副作用 crate 边界，不改变现有行为。
3. `B2` 在 `R2` 通过后接通 Tetris GPU 入口，完成 Terminal/GPU 双后端本地验收。
4. `F3` 先批准 provisional UI `PEP 0002`，再实现无状态布局、只读文本、焦点和基础 widget。
5. `B3` 接受 Punctum 与 Tetris 的 UI 依赖变化，验证 UI crate 不依赖 backend 或产品类型。

`GPU-REF-v0.1` 继续阻塞固定环境 readback 和正式 release oracle，但不阻塞 `R2` adapter 边界、`B2` 双后端 Tetris 或 `F3` UI foundation。

`R2` 的具体实施任务、写入所有权和并行方式在启动前单独确定。本文件只固定阶段目标和门禁，不提前指定 Agent。
...
- 本轮不建设 CI。`PT1` 与 `F2` 已通过，Terminal Unicode 文本合同的 28 项测试已通过。GPU pure planner、wgpu runtime、winit 输入适配和本机 headless smoke 已通过。GPU 当前 coverage 结果为 560/560 regions、37/37 functions、460/460 lines；下一步先进入 `R2`，再在 `B2` 接通 Tetris GPU 入口。
