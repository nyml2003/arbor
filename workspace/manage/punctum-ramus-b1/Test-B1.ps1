...
function Get-ExportSha256([string[]] $CrateDirectories) {
    ...
    $relativePaths = Get-OrdinalSortedStrings @($files | ForEach-Object {
...
foreach ($retiredPath in $Records.retired_paths) {
    if (Test-Path -LiteralPath (Join-Path $RepoRoot $retiredPath)) {
        $Failures.Add("Retired path still exists: $retiredPath")
    }
}
...
foreach ($workspaceProperty in $Records.workspaces.PSObject.Properties) {
    ...
    if ($LASTEXITCODE -ne 0) {
        $Failures.Add("cargo metadata failed for $workspaceName with exit code $LASTEXITCODE")
        continue
    }
    $metadata = ($metadataJson -join "`n") | ConvertFrom-Json
    $MetadataByWorkspace[$workspaceName] = $metadata

    Assert-Equal "$workspaceName workspace_root" `
        (Get-RepoRelativePath $metadata.workspace_root) `
        (Get-RepoRelativePath $manifestDirectory)
    Assert-Equal "$workspaceName root_manifest_sha256" `
        (Get-FileSha256 $manifestPath) `
        $baseline.root_manifest_sha256
    Assert-Equal "$workspaceName lockfile_sha256" `
        (Get-FileSha256 (Join-Path $manifestDirectory "Cargo.lock")) `
        $baseline.lockfile_sha256

    $memberPackages = @($metadata.packages | Where-Object {
    ...
    Assert-Equal "$workspaceName sorted_member_list_sha256" `
        (Get-TextSha256 $memberListText) `
        $baseline.sorted_member_list_sha256

    $actualManifestPaths = Get-OrdinalSortedStrings @($memberPackages | ForEach-Object {
        Get-RepoRelativePath $_.manifest_path
    })
    $expectedManifestPaths = Get-OrdinalSortedStrings @(
        $baseline.member_manifest_sha256_by_path.PSObject.Properties.Name
    )
    Assert-Equal "$workspaceName member manifest paths" `
        ($actualManifestPaths -join "|") `
    ...
    foreach ($manifestProperty in $baseline.member_manifest_sha256_by_path.PSObject.Properties) {
        ...
        Assert-Equal "$workspaceName member manifest $($manifestProperty.Name)" `
            (Get-FileSha256 $memberManifest) `
    ...
    Assert-Equal "$workspaceName approved_upstream_export_sha256" `
        (Get-ExportSha256 @($baseline.approved_upstream_crates)) `
...
foreach ($pathRecord in $Records.canonical_path_dependencies) {
    ...
    $approvedTarget = Get-CanonicalRepoPath (
        Join-Path $RepoRoot $pathRecord.approved_repo_relative_target
    )
    Assert-Equal "$($pathRecord.consumer) $($pathRecord.dependency) declared path" `
        $declaredTarget `
        $approvedTarget
    ...
    foreach ($dependency in $resolvedDependencies) {
        Assert-Equal "$($pathRecord.consumer) $($pathRecord.dependency) metadata path" `
            (Get-CanonicalRepoPath $dependency.path) `
            $approvedTarget
    }
}

foreach ($exportProperty in $Records.exports.PSObject.Properties) {
    $export = $exportProperty.Value
    Assert-Equal "export $($exportProperty.Name)" `
        (Get-ExportSha256 @($export.crates)) `
        $export.sha256
}
...
Assert-Equal "independent lockfile path count" `
    @($lockfiles | Sort-Object -Unique).Count `
    $lockfiles.Count
...
$allowedPaths = @(
    ...
    "apps/gen3-game/",
    "apps/tui-chater/",
    "workspace/manage/punctum-ramus-",
    "workspace/manage/punctum-vsh-program.md",
    "workspace/manage/punctum-vsh-architecture-plan.md",
    "workspace/manage/punctum-vsh-s0/records.json"
)
foreach ($statusLine in @(& git -C $RepoRoot status --short --untracked-files=all)) {
    ...
    if (-not ($allowedPaths | Where-Object {
        $changedPath -eq $_ -or $changedPath.StartsWith($_, [StringComparison]::Ordinal)
    })) {
        $Failures.Add("Changed path is outside the B1 write scope: $changedPath")
    }
}
...
Write-Output "B1 verification passed"
exit 0
