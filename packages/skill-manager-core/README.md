...
### 7. Arbor 自维护 Skill 集合

Arbor 可以把自维护 Skill 放在一个 workspace 子项目里，用 skill manager 管理自己。

```text
packages/arbor-skills/
  arbor.skills.json
  skills/
    plain-tech-writing-cn/
      SKILL.md
      skill.package.json
    skill-manager-maintainer/
      SKILL.md
      skill.package.json
```

`arbor.skills.json` 使用本地 path source，`targetDir` 指向 `.installed-skills`。这样可以先验证 Skill 结构、manifest、lock 和安装流程，再决定是否拆成独立 GitHub 仓库。
