...
impl Text {
    pub fn new(content: impl Into<String>) -> Self {
        Self { content: content.into(), fg: None, bg: None, attrs: Attrs::default(), padding: RectOffset::default(), flex: 0.0, width: None, wrap: WrapStrategy::None, truncate: TruncateStrategy::End }
    }
    ...
    pub fn build(self, wm: &WidgetManager, t: &Theme) -> WidgetNode {
        let style = TextStyle {
        ...
        wm.wrap(|id| TextWidget {
            id, props: LayoutProps { padding: self.padding, flex: self.flex, width: self.width, ..Default::default() },
            text: ReadSignal::constant(self.content),
            style: ReadSignal::constant(style),
            wrap: self.wrap, truncate: self.truncate,
        })
    }
}
