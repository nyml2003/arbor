...
impl Scroll {
    pub fn new() -> Self { Self { child: None, padding: RectOffset::default(), flex: 0.0, content_h: 0, scroll_y: None } }
    ...
    pub fn content_h(mut self, h: u16) -> Self { self.content_h = h; self }
    pub fn scroll_y(mut self, s: ReadSignal<u16>) -> Self { self.scroll_y = Some(s); self }
    pub fn build(self, wm: &WidgetManager, _t: &arbor_tui_render::theme::Theme) -> WidgetNode {
        ...
        wm.wrap(|id| ScrollViewWidget {
            id, props: LayoutProps { padding: self.padding, flex: self.flex, ..Default::default() },
            child: Box::new(child),
