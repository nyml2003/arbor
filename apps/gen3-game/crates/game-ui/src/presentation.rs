...
impl PresentationState {
    ...
    fn advance_mut(&mut self, elapsed: Duration, game: &GameSnapshot) -> PresentationUpdate {
        ...
        if take_elapsed(&mut self.run_stop_remaining, elapsed) {
            redraw = true;
        }
        PresentationUpdate {
            redraw,
    ...
    fn handle_console_key(
        ...
        match self.console.handle(intent) {
            ...
            ConsoleOutcome::Updated | ConsoleOutcome::NoSelection => PresentationUpdate::redraw(),
            ConsoleOutcome::Ignored => PresentationUpdate::default(),
        }
    }
...
impl WorldMotion {
    const fn new(direction: Direction, gait: Gait) -> Self {
        Self {
            direction,
            gait,
...
mod tests {
    ...
    fn console_ime_and_keyboard_paths_are_explicit_updates() {
        ...
        assert_eq!(state.console_view().diagnostic.as_deref(), Some("runtime failed"));
    ...
    fn timers_motion_and_direction_helpers_cover_all_boundaries() {
        ...
        assert!(!take_elapsed(&mut timer, Duration::from_millis(4)));
        assert_eq!(timer, Some(Duration::from_millis(6)));
        assert!(take_elapsed(&mut timer, Duration::from_millis(6)));
        ...
        let mut periodic = Some(Duration::from_millis(10));
        assert!(!advance_periodic(
            &mut periodic,
        ...
        assert_eq!(console_intent_for_key(&key(NamedKey::ArrowUp)), Some(ConsoleIntent::Previous));
        assert_eq!(console_intent_for_key(&key(NamedKey::ArrowDown)), Some(ConsoleIntent::Next));
        assert_eq!(console_intent_for_key(&key(NamedKey::Backspace)), Some(ConsoleIntent::Backspace));
        assert_eq!(console_intent_for_key(&key(NamedKey::Enter)), Some(ConsoleIntent::Execute));
        assert_eq!(console_intent_for_key(&key(NamedKey::Escape)), Some(ConsoleIntent::Close));
