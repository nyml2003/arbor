...
pub fn type_icon_asset(pokemon_type: PokemonType) -> AssetKey {
    ...
    AssetKey::new(format!("battle/type/{name}")).expect("type icon asset keys are non-empty")
}
...
pub fn move_category_icon_asset(category: MoveCategory) -> AssetKey {
    ...
    AssetKey::new(format!("battle/move-category/{name}"))
...
mod tests {
    ...
    fn battle_session_with_move(
        ...
        fn team(prefix: &str, move_type: PokemonType, current_pp: u8, accuracy: Accuracy) -> Team {
        ...
        let application = BattleApplication::new(
            team("己方", PokemonType::Grass, current_pp, accuracy),
            team("对手", PokemonType::Fire, current_pp, accuracy),
            42,
        )
        .unwrap();
    ...
    fn product_assets_use_stable_semantic_keys() {
        ...
        for category in [
            ...
            assert!(
                move_category_icon_asset(category)
                    .as_str()
                    .starts_with("battle/move-category/")
            );
        }
        for direction in [
            Direction::Down,
