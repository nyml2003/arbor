//! Development-time PokeAPI CSV importer with structured diagnostics.
...
fn validate_source_metadata(source: &Path, commit: &str) -> Result<(), ImportFailure> {
    let path = source.join("SOURCE.md");
    let contents = fs::read_to_string(&path).map_err(|error| {
        ImportFailure::one(
            ImportDiagnostic::new(
                ImportDiagnosticCode::MissingInputFile,
                format!("cannot read source metadata: {error}"),
            )
            .file(&path),
        )
    })?;
...
pub fn import_directory(
    ...
    let language_id = languages
        ...
        .ok_or_else(|| {
            ImportFailure::one(
                ...
                .file(source.join("languages.csv"))
                .field("identifier"),
            )
        })?;
    let version_groups =
    ...
    let version_group_id = version_groups
        ...
        .ok_or_else(|| {
            ImportFailure::one(
                ...
                .file(source.join("version_groups.csv"))
                .field("identifier"),
            )
        })?;
    ...
    let moves_path = source.join("moves.csv");
    ...
    let stats_path = source.join("pokemon_stats.csv");
    ...
    let types_path = source.join("pokemon_types.csv");
    ...
    let pokemon_path = source.join("pokemon.csv");
    ...
    let learnsets_path = source.join("pokemon_moves.csv");
...
pub fn generate_to_path(
    ...
    let bytes = serde_json::to_vec_pretty(&dataset).map_err(|error| {
        ImportFailure::one(ImportDiagnostic::new(
            ImportDiagnosticCode::OutputValidationFailed,
            format!("cannot serialize generated data: {error}"),
        ))
    })?;
    CurrentDataSet::from_json(&bytes).map_err(|error| {
        ImportFailure::one(ImportDiagnostic::new(
            ImportDiagnosticCode::OutputValidationFailed,
            format!("generated output failed validation: {error}"),
        ))
    })?;
    ...
    if output.exists() {
        fs::remove_file(output).map_err(|error| {
            ImportFailure::one(
                ImportDiagnostic::new(
                    ImportDiagnosticCode::OutputValidationFailed,
                    format!("cannot replace existing output: {error}"),
                )
                .file(output),
            )
        })?;
    ...
    fs::rename(&temporary, output).map_err(|error| {
        ImportFailure::one(
            ImportDiagnostic::new(
                ImportDiagnosticCode::OutputValidationFailed,
                format!("cannot publish generated output: {error}"),
            )
            .file(output),
        )
    })?;
