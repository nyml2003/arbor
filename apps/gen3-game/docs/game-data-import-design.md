...
`game-assets` 仍只处理 PNG 解码和 GPU atlas。游戏数据由独立 crate 负责。`game-host` 尚未接入 `CurrentDataSet`，仍直接构造妙蛙种子和招式。
...
- `schema_version`，初始值为 `current-data-set-v1`。
- `source_repository`。
- `source_commit`。
- `generator_version`。
- `locale`，初始值为 `zh-Hans`。
...
`embedded()` 使用 `include_bytes!("../../../assets/data/current-data-set-v1.json")`。它只解析已生成资源。它不读取原始 CSV。

第一阶段可以使用 JSON 和 `serde_json`。数据量和启动开销有证据表明成为问题后，再改成二进制格式。schema 版本必须在格式变化时更新。
...
3. 从 `stats.csv`、`types.csv` 和 `move_damage_classes.csv` 建立稳定标识映射。
...
当前 `battle-domain::Move` 要求威力大于零，因此变化招式不能直接构造。第一阶段的战斗投影应返回结构化的“不支持”错误，不应伪造威力。

当前战斗规则按第三世代属性决定物理或特殊分类，PokeAPI 当前数据则包含现代招式伤害类别。`CurrentDataSet` 保留现代数据事实。第三世代战斗投影必须明确使用第三世代规则，不能悄悄覆盖静态记录，也不能把投影结果写回数据集。

建议先完成 `game-data` 查询，不立即替换 `demo_bulbasaur`。接入战斗时，在 application 或单独 adapter 中增加显式构造服务。`game-data` 不依赖 `battle-domain`。
...
cargo run -p game-data-import -- `
  ...
  --output assets/data/current-data-set-v1.json `
  --locale zh-Hans `
...
`game-host` 在组合根加载 `CurrentDataSet`。先用查询结果展示妙蛙种子和两个招式，再设计完整队伍与学习面。

完成标准：删掉 `demo_bulbasaur` 中重复的名称、属性、种族值、威力和 PP 常量；程序仍能完全离线启动。

### 阶段 5：扩展数据范围

按具体 `version_group` 加入学习面，然后分批加入特性、道具和复杂招式效果。每次扩展都升级 schema 或保持向后兼容，并增加导入校验。
...
```powershell
cargo fmt --all -- --check
cargo test -p game-data -p game-data-import
cargo clippy -p game-data -p game-data-import --all-targets -- -D warnings
```

## 暂不决定

以下问题等第一阶段数据量和查询使用方式明确后再决定：
