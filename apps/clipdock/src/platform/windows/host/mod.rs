...
use windows::Win32::UI::WindowsAndMessaging::{
    ...
    SWP_SHOWWINDOW, SW_SHOWNA, WM_CANCELMODE, WM_CAPTURECHANGED, WM_CLIPBOARDUPDATE, WM_CLOSE,
    WM_CREATE, WM_DESTROY, WM_DPICHANGED, WM_ERASEBKGND, WM_LBUTTONDOWN, WM_LBUTTONUP,
    WM_MOUSEMOVE, WM_NCCREATE, WM_NCHITTEST, WM_PAINT, WM_SIZE, WM_TIMER, WS_EX_NOACTIVATE,
    WS_EX_TOOLWINDOW, WS_EX_TOPMOST, WS_POPUP,
};

use crate::app::{AppCommand, ChromeHit, ClipDockApp, Point, PointerEvent, Size};
...
const CLASS_NAME: windows::core::PCWSTR = w!("ClipDockWindowClass");
const WINDOW_TITLE: windows::core::PCWSTR = w!("ClipDock");
const INITIAL_WIDTH: i32 = 420;
const INITIAL_HEIGHT: i32 = 520;
...
impl ClipDockWindow {
    pub fn new() -> PlatformResult<Self> {
        ...
        // SAFETY: The class is registered in this module, strings are static UTF-16, and state_ptr
        // points to boxed state that outlives the HWND.
        let hwnd = unsafe {
            CreateWindowExW(
            ...
            .context("create clipdock window")?
        };

        state.attach(hwnd)?;

        // SAFETY: hwnd is live and owned by this wrapper. SWP_NOACTIVATE preserves the dock behavior.
        unsafe {
            SetWindowPos(
                hwnd,
                Some(HWND_TOPMOST),
            ...
            .context("show topmost clipdock window")?;
    ...
    pub fn run(&mut self) -> PlatformResult<()> {
        ...
        loop {
            // SAFETY: message points to initialized writable storage and loop runs on UI thread.
...
impl WindowState {
    ...
    fn attach(&mut self, hwnd: HWND) -> PlatformResult<()> {
        ...
        clipboard::add_listener(hwnd)?;
        let size = client_size(hwnd)?;
        self.app.resize(size)?;
        self.capture_clipboard(hwnd)?;
        Ok(())
    }
    ...
    fn handle_message(
        ...
        match message {
            ...
            WM_CLIPBOARDUPDATE => {
                let _ = self.capture_clipboard(hwnd);
                LRESULT(0)
            }
            WM_MOUSEMOVE => {
            ...
            WM_LBUTTONUP => {
                ...
                if let Ok(update) = self.app.handle_pointer_event(PointerEvent::Up(point)) {
                    ...
                    if update.commands.contains(&AppCommand::CloseApp) {
                        return LRESULT(0);
                    }
                }
                self.sync_animation_timer(hwnd);
            ...
            WM_DESTROY => {
                ...
                unsafe {
                    clipboard::remove_listener(hwnd);
    ...
    fn capture_clipboard(&mut self, hwnd: HWND) -> PlatformResult<()> {
        if let Some(text) = clipboard::read_text(hwnd)? {
            if self.app.record_clipboard_text(text)? {
                self.render(hwnd)?;
            }
        }
        Ok(())
    ...
    fn hit_test(&self, hwnd: HWND, lparam: LPARAM) -> LRESULT {
        // SAFETY: WM_NCHITTEST defaults must run first so Windows can keep system behavior it owns.
    ...
    fn execute_commands(&self, hwnd: HWND, commands: &[AppCommand]) {
        for command in commands {
            match command {
                AppCommand::CloseApp => {
                    // SAFETY: CloseApp is generated by ClipDock's own UI and destroys its HWND.
                    let _ = unsafe { DestroyWindow(hwnd) };
                }
                AppCommand::PasteText(text) => {
                    let _ = clipboard::write_text(hwnd, text);
                    let _ = input::paste_shortcut();
                }
            }
        }
    }
...
fn register_window_class(instance: HINSTANCE) -> PlatformResult<()> {
    ...
    // SAFETY: class matches the Win32 WNDCLASSEXW ABI. Background brush is null because Direct2D
    // owns painting and WM_ERASEBKGND is suppressed.
    let atom = unsafe { RegisterClassExW(&class) };
    if atom == 0 {
        return Err(windows::core::Error::from_thread()).context("register clipdock window class");
    }
    Ok(())
}
...
fn size_from_rect(rect: arbor_ui_core::Rect) -> Size {
...
extern "system" fn window_proc(
    ...
    if message == WM_NCCREATE {
        ...
        // WindowState pointer passed to CreateWindowExW and remains boxed by ClipDockWindow.
        unsafe {
    ...
    // SAFETY: state_ptr is owned by ClipDockWindow and lives at least as long as the HWND.
